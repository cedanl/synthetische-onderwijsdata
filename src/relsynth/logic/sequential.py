"""
State-aware sequential generator for longitudinal records.

Rather than flattening trajectories (which loses temporal semantics), this
module generates each time step by applying DBN transitions from the previous
state.  The number of steps per entity is drawn from a DegreeModel.
"""
...
if TYPE_CHECKING:
    ...
    from relsynth.presets.loader import TableSchema
...
class SequentialGenerator:
    ...
    def fit(self, data: pd.DataFrame) -> "SequentialGenerator":
        seq_cfg = self._schema.sequential
        entity_key: str = seq_cfg["entity_key"]  # type: ignore[index]
        time_key: Optional[str] = seq_cfg.get("time_key")  # type: ignore[union-attr]

        cat_cols = [
            c
            for c, col in self._schema.columns.items()
        ...
        self._transition = TransitionModel(random_state=seed)
    ...
    def generate(
        ...
        n_entities = len(parent_df)
        degrees = (
            degree_model.sample(n_entities)
            if degree_model is not None
            else np.full(n_entities, 3, dtype=int)
        )
